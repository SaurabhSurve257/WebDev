    ...
    try {
      ...
      navigate('/patient/confirmation', {
        state: {
          ...
          slot: selectedSlot,
          reason,
          appointment: appointmentResponse?.appointment || null,
        },
      })
    } catch (requestError) {
  ...
  if (!doctor) {
    return (
      <section className="min-h-dvh px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto w-full max-w-3xl rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-xl shadow-slate-900/10 sm:p-8">
          <h1 className="text-3xl font-bold text-slate-900">No Doctor Selected</h1>
          <p className="mt-2 text-slate-600">
            Please return to the doctor listing and choose a doctor to continue.
          </p>
          <Link
            to="/patient/doctors"
            className="mt-5 inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-teal-600 to-teal-700 px-4 py-2.5 font-semibold text-white shadow-lg shadow-teal-700/25 transition hover:from-teal-700 hover:to-teal-800"
          >
            Back to Doctors
          </Link>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
          <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/10">
            ...
            <div className="mt-4 space-y-2 text-sm text-slate-600">
              <p>
                <span className="font-semibold text-slate-700">Age:</span> {doctor.age ?? 'N/A'}
              </p>
              <p>
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-xl px-3 py-2 text-sm font-semibold transition-all duration-200 ${
                          active
                            ? 'border border-teal-700 bg-gradient-to-r from-teal-600 to-emerald-600 text-white shadow-lg shadow-teal-700/25 ring-2 ring-teal-200'
